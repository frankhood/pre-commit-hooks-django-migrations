...
def coverage(c):
    ...
    """
    c.run("coverage run --source pre-commit-hooks-django-migrations runtests.py tests")
...
def docs(c):
    ...
    """
    c.run("rm -f docs/pre-commit-hooks-django-migrations.rst")
    c.run("rm -f docs/modules.rst")
    c.run("sphinx-apidoc -o docs/ pre_commit_hooks_django_migrations")
...
def lint(c):
    ...
    """
    c.run("flake8 pre-commit-hooks-django-migrations tests")
...
def release(c, bumpsize=''):
    ...
    c.run('git tag -a {version} -m "New version: {version}"'.format(version=hooks.__version__))
    c.run("git push --tags")
